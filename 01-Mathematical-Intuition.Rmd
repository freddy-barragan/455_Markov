...
Let $X_n = X_1, \dots, X_{n-1}, X_n$ be a collection of successive time-indexed event outcomes. 

The $X_i$ are a discrete **Markov Chain** if 


- They exhibit a **Markov Property**.
  + Where the probability of some new or predicted event $X_{n+1}=x_{n+1}$ is 

$$P(X_{n+1} = x_{n+1}|X_1 = x_1, \dots,  X_{n-1}= x_{n-1}, X_n= x_{n}) = P(X_{n+1} = x_{n+1}|X_n = x_n)$$
...
- There is some countable set of outcomes called the **State Space** which contains every possible outcome or **State** 
  + The outcomes $x_1, \dots, x_{n-1}, x_{n}, x_{n+1}$ are all common elements of $\mathbb{S}$.
